...
// rule applied to edges. Graph-wave lane B: `derivation` (EXTRACTED|INFERRED) is now a SECOND,
// higher-priority dash rule layered on top of that per-kind one (lineDashFor) — any INFERRED
// edge (today, only HOT_PATH — dev.fonebrew.domain.thread.ThreadGraphAnalytics's own contract)
// draws the same fine dash regardless of its kind's own pattern, mirroring GraphRoom.kt's native
// canvas rule of the same name; an EXTRACTED (or unlabelled, pre-1.2.0) edge is unaffected. Lane B
// also wires an edge click inspector (`edge:click` -> renderEdgeInspector) alongside the existing
// node one — kind + from/to + derivation + `because`, always as TEXT (binding constraint 6).
//
...
(function () {
  ...
  // Human vocabulary for a raw ThreadEdgeKind token (graph-wave lane B) — mirrors GraphRoom.kt's
  // own edgeKindLabel(), for the edge inspector below.
  var EDGE_KIND_LABEL = {
    REPLY: 'Reply',
    FORK: 'Fork',
    SPAWN: 'Spawn',
    ...
    DELEGATION_ANCHOR: 'Delegation anchor',
    COMMIT_ANCHOR: 'Commit anchor',
    HOT_PATH: 'Hot path',
  };

  // Vocabulary for ThreadGraphEdge.derivation (graph-wave lane B) — mirrors GraphRoom.kt's own
  // derivationLabel(): the WCAG-safe TEXT channel alongside the dash/solid line style
  // lineDashFor draws (binding constraint 6 — never dash-only).
  var DERIVATION_LABEL = { EXTRACTED: 'Extracted — restates a recorded fact', INFERRED: 'Inferred — computed from recorded structure' };
  ...
  function edgeStyleFor(kind, theme) {
    switch (kind) {
      ...
      // HOT_PATH (graph-wave lane B): dev.fonebrew.domain.thread.ThreadGraphAnalytics's own derived
      // sibling link — always 'INFERRED' in practice, so DERIVATION_DASH below is what actually
      // draws for it; this entry only supplies the fallback colour + a defensive dash pattern for
      // the hypothetical of a HOT_PATH edge some future caller minted with no derivation at all.
      case 'HOT_PATH': return { stroke: theme.textMid, lineDash: [2, 3] };
      default: return { stroke: theme.textMid, lineDash: null };
    }
  }

  // Graph-wave lane B (binding constraint 6 / WCAG 1.4.1): 'derivation' is the PRIMARY dash
  // channel — any INFERRED edge draws this fine dash regardless of the kind's own pattern above,
  // so "recorded vs derived" reads the same way at a glance as GraphRoom.kt's native canvas draws
  // it (that file's own ThreadMapCanvas comment names the identical rule). An EXTRACTED (or
  // unlabelled, pre-1.2.0) edge keeps its existing per-KIND pattern from edgeStyleFor, unchanged.
  var DERIVATION_DASH = [2, 3];
  function lineDashFor(kind, derivation, theme) {
    if (derivation === 'INFERRED') return DERIVATION_DASH;
    return edgeStyleFor(kind, theme).lineDash;
  }
  ...
  function toGraphData(g, theme) {
    ...
    const edges = (g.edges || []).map(function (e, i) {
      ...
      return {
        ...
        source: e.from,
        target: e.to,
        // derivation/because (graph-wave lane B): lineDashFor makes derivation the PRIMARY dash
        // channel (any INFERRED edge — today, HOT_PATH only — draws the same fine dash regardless
        // of the kind's own pattern), mirroring GraphRoom.kt's native canvas rule of the same name.
        style: { stroke: st.stroke, lineDash: lineDashFor(e.kind, e.derivation, theme), lineWidth: lineWidth, strokeOpacity: strokeOpacity },
        // derivation/because (1.2.0, graph-wave lane A; edge click inspector wired graph-wave lane
        // B — see renderEdgeInspector/'edge:click' below): carried onto the G6 edge's own data,
        // same as every other field here. Absent (undefined) for an edge from a pre-1.2.0 snapshot
        // or one this producer never populated, never a fabricated placeholder.
  ...
  // Graph-wave lane B: same "read straight from the last parsed ThreadGraph JSON, never a G6
  // internal lookup" rationale as lastGraphNodesById above, keyed the same way toGraphData's own
  // edges.map assigns ids ('e' + index).
  var lastGraphEdgesById = {};
  ...
  // Graph-wave lane B: the edge counterpart to renderInspector above — GraphRoom.kt's native
  // EdgeDetailsDialog is the precedent (kind + from/to + derivation + because, always as TEXT,
  // never left to the dash-vs-solid line style alone — binding constraint 6). Shares the same
  // #graph-room-inspector element (mutually exclusive with a node selection); a fresh load() or a
  // canvas click clears it via the same renderInspector(null) call.
  function renderEdgeInspector(edge) {
    var el = document.getElementById('graph-room-inspector');
    if (!el) return;
    if (!edge) { el.hidden = true; el.textContent = ''; return; }
    var lines = [(EDGE_KIND_LABEL[edge.kind] || edge.kind)];
    // → escaped (not a literal arrow char), matching this bootstrap's own '\\u00b7' status-
    // line convention below — this string is authored inside a JS template literal that is itself
    // interpolated into an outer HTML template literal, so an escaped codepoint is the safer
    // round-trip than a literal multi-byte character here.
    lines.push(String(edge.from) + ' \\u2192 ' + String(edge.to));
    if (edge.derivation) lines.push(DERIVATION_LABEL[edge.derivation] || edge.derivation);
    if (edge.because) lines.push(String(edge.because));
    el.textContent = lines.join('\\n');
    el.hidden = false;
  }
  ...
  function ensureGraph() {
    ...
    // Graph-wave lane B: the edge counterpart — 'edge:click' is the same real graph-level event
    // this vendored build's click-select behavior keys off internally for edges, mirroring
    // 'node:click' above exactly (evt.target.id, try/catch, best-effort).
    graph.on('edge:click', function (evt) {
      try {
        var id = evt && evt.target && evt.target.id;
        renderEdgeInspector(id ? lastGraphEdgesById[id] : null);
      } catch (err) { /* inspector is best-effort; the graph itself must keep working */ }
    });
  ...
  window.__graphRoom = {
    load: function (jsonText, themeJsonText) {
      try {
        ...
        // Graph-wave lane B: keyed 'e' + index — the exact id scheme toGraphData's own edges.map
        // assigns each G6 edge, so 'edge:click's evt.target.id looks itself up here correctly.
        lastGraphEdgesById = {};
        (parsed.edges || []).forEach(function (e, i) { lastGraphEdgesById['e' + i] = e; });
