// JNI bridge for a real, PTY-backed local shell (dev.aarso.data.remote.NativePty /
...
Java_dev_aarso_data_remote_NativePty_spawn(
...
Java_dev_aarso_data_remote_NativePty_resize(JNIEnv* /*env*/, jobject /*thiz*/, jint fd, jint rows, jint cols) {
    struct winsize ws{};
    ws.ws_row = (unsigned short) rows;
    ws.ws_col = (unsigned short) cols;
...
Java_dev_aarso_data_remote_NativePty_waitFor(JNIEnv* /*env*/, jobject /*thiz*/, jint pid) {
    int status = 0;
