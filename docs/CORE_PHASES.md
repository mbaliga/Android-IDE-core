...
7. **Brand string is "Fonebrew"** (owner ruling 2026-08-01, permanent unless the owner explicitly says otherwise — supersedes the interim "FoneBru" ruling of 2026-07-11). Package rename stays deferred (Sprint R) — do not touch Gradle identifiers.
...
**P4 — Conversation Instrument.** Per-message annotations on top of the existing message tree, additive only (no tree/entity already shipped is touched): `Verdict` (a human judgment, one live per message, annotation-only — it never moves, files, or archives a message anywhere in the tree, by design), `MessageBookmark` (a new, message-level tier, distinct from the existing conversation-level "Starred" bookmark — both coexist), `Version` (a named milestone on a branch), `CompactionDirective` (a per-message fidelity dial — F0 droppable through F3 verbatim-required — plus a "must-include" flag), `GhostBranch` (an annotation marking a branch inactive after a rewind — nothing is ever deleted; the branch stays reachable via the existing alternative-branch navigation), `FormState` (stored questionnaire answers, enabling a rewound form to reopen pre-filled). A deterministic contract resolves each message's fidelity from its verdict/bookmark/version-spine signals, with mechanical post-run verification that every verbatim-required span survived byte-for-byte — a run that violates this fails loudly rather than shipping silently. Two new inert seams (mirroring the existing `ProjectRoomSlot`/`DevelopTabs` pattern — not "CallSlot," which doesn't correspond to any class in this codebase) let an above-core layer install a multi-model compare surface and a version-suggestion surface later, without core knowing anything about either. A new, off-by-default, write-only local event log records these interactions for a future personal-reflection feature that is otherwise completely inert — no reading, computing, or displaying happens anywhere in this phase.
DoD: 80 new JVM tests (data model + merge-shape logic, the fidelity-resolution contract incl. its documented precedence rules, the mechanical verbatim-verification check incl. a deliberately-tampered-agent test proving it's caught, a concurrent-toggle race regression test, a newest-wins version-tie-break regression test, an out-of-batch-directive receipt-scoping regression test, the two new slots' snapshot-recomposition mechanics, the event log's per-signal capture toggle and JSONL line shape, a real-file append test) [code-complete + verified-JVM]; full existing gate (both flavors) still green, no regressions; message-bubble rating/bookmark controls and the extended long-press sheet are code-complete and compile-verified, but all render/gesture/haptic behaviour is [owner-verify] per the standing environment-honesty rule — this container has no device to confirm any of it renders or feels right. A background adversarial multi-agent review of the full phase diff found 7 real bugs (a stale-fidelity regression on toggling must-include, a bookmark-toggle check-then-act race, a directive-count receipt scoping bug, a missing version-tie-break ORDER BY, an undocumented verdict-vs-spine compaction precedence, a role-gated bookmark row that silently hid a role-agnostic gesture's feedback, and a blocking-I/O-on-caller-dispatcher event-sink bug) — all 7 fixed in the same phase before merge.
