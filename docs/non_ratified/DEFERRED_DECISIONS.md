...
| `FB-RAT-LBX-008` | Paired direct transfer | Defer encrypted browser-to-phone pairing and nearby transfer until file, URL, QR, and Git transfers pass conformance. | File/URL/QR/Git transfer channels (`schemas/loops/transfer-envelope.schema.json`) pass their conformance suite. | DEFERRED |
| `FB-RAT-LBX-009` | Final browser product naming | Defer final choice among fonebrew.com, fonebrew.dev, or a subdomain until marketplace and account boundaries are ratified. | Marketplace and account boundaries (`FB-RAT-MKT-001`, `FB-RAT-MKT-009`) are ratified by the owner. | DEFERRED |
| `FB-RAT-WEB-008` | Browser hardware execution | Defer WebUSB/WebSerial/WebBluetooth execution and flashing until phone runtime and marketplace trust models are stable. | Phone runtime (`DEVICE_STATE_AND_SAFETY_SPEC.md`) and marketplace trust models are both stable and owner-reviewed. | DEFERRED |
| `FB-RAT-IMP-010` | One-tap install from trusted creator | Defer any reduced-confirmation install flow until publisher trust, authority diff, and abuse gates are proven. | Publisher trust (signature/identity), `authority-diff.schema.json` detection, and marketplace abuse gates are all proven in production use. | DEFERRED |
| `FB-RAT-MKT-010` | Creator monetization | Defer paid loops, tipping, revenue share, and commercial licenses until moderation, refunds, trust, and legal terms are designed. | Real legal review of DSA Section 4 trader traceability, consumer withdrawal rights, and payment/tax registration (per `10_DUAL_VALIDATION_ADDENDUM.md` §G) — owner-only, cannot be done in-session. | DEFERRED |
| `FB-RAT-MKT-011` | Personalized recommendations | Defer personalized recommendations; P1 discovery uses explicit filters, categories, compatibility, and curation. | Explicit-filter discovery (P1 registry/browse) ships and is observed in use before any ranking/recommendation model is considered. | DEFERRED |
| `FB-RAT-RES-007` | Public aggregate success rates | Defer public aggregate success, intervention, and failure statistics until sampling bias and verification semantics are defined. | Sampling-bias methodology and verification semantics (building on the `FB-RAT-RES-004` `SELF_SIGNED_RECEIPT` correction) are defined and owner-reviewed. | DEFERRED |
| `FB-RAT-LIN-007` | Automated semantic merge | Defer automatic graph merging; v1 supports comparison and selective application with validation after every operation. | Comparison + selective-application (`loop-semantic-diff.schema.json`) is proven reliable in production use across enough real forks to justify automating any part of it. | DEFERRED |
