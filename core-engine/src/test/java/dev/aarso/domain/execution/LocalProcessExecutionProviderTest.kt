...
class LocalProcessExecutionProviderTest {
    ...
    fun `reconnect -- a known token returns the current handle state`() = runBlocking {
        ...
        assertEquals(dev.aarso.contracts.execution.ReconnectOutcome.ESTABLISHED, reconnected.reconnectAttempt!!.outcome)
    }
    ...
    fun `descriptor declares LOCAL_ANDROID support`() {
        val provider = LocalProcessExecutionProvider()
        assertTrue(dev.aarso.contracts.execution.ExecutionTargetType.LOCAL_ANDROID in provider.descriptor.supportedTargetTypes)
    }
}
