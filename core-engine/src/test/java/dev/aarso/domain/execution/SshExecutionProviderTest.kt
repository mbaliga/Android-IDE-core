 ...
 * [dev.aarso.domain.remote.RemoteSessionDriver]/[dev.aarso.domain.remote.SessionMachine]/
 * [dev.aarso.domain.remote.KnownHosts] orchestration logic against a fake I/O layer only.
 */
...
class SshExecutionProviderTest {
    ...
    private fun provider(transport: RemoteTransport, approveTrust: suspend (dev.aarso.domain.remote.Trust) -> Boolean = { true }) =
    ...
    fun `descriptor declares both SSH_HOST and RASPBERRY_PI support`() {
        val provider = provider(FakeRemoteTransport())
        assertTrue(dev.aarso.contracts.execution.ExecutionTargetType.SSH_HOST in provider.descriptor.supportedTargetTypes)
        assertTrue(dev.aarso.contracts.execution.ExecutionTargetType.RASPBERRY_PI in provider.descriptor.supportedTargetTypes)
    }
}
