 ...
 * the properties `dev.aarso.ui.graph.GraphWebRoom`'s WebView lockdown depends on, without a
 ...
 * [dev.aarso.domain.thread.ThreadGraphJsonTest] has to its embedded fixture. If this file is ever
...
class GraphRoomAssetTest {
    ...
    @Test fun `the __graphRoom load entry point is defined`() {
        ...
        // Named in dev.aarso.domain.thread.ThreadGraphJson's own KDoc as this exact call's
    ...
    @Test fun `node kind maps to a distinct built-in shape per kind, never colour alone`() {
        ...
        // (dev.aarso.domain.thread.ThreadNodeKind) must map to its own G6 built-in node type
