...
class IntegrationsCodecTest {
    ...
    fun `decodes the real WP-1 assay-index-valid fixture exactly`() {
        ...
        assertEquals(dev.fonebrew.contracts.integrations.AssayCompleteness.COMPLETE, index.completeness)
    }
    ...
    fun `decodes the real WP-1 import-receipt-baseline-valid fixture exactly`() {
        ...
        assertEquals(dev.fonebrew.contracts.integrations.ImportDecision.FULL, receipt.decision)
    ...
    fun `ImportErrorCode wire round-trip covers the fixed set and the IMPORT_INVALID_ FIELD family`() {
        assertEquals("IMPORT_UNSUPPORTED_MAJOR", IntegrationsCodec.encodeErrorCode(dev.fonebrew.contracts.integrations.ImportErrorCode.UnsupportedMajor))
        val invalidField = dev.fonebrew.contracts.integrations.ImportErrorCode.InvalidField("SEVERITY")
