...
class SecretRedactionScannerTest {
    ...
    fun `receipt logs excerpt produced by LocalProcessExecutionProvider for a benign echo carries no secret-shaped leak`() = kotlinx.coroutines.runBlocking {
        val provider = dev.fonebrew.domain.execution.LocalProcessExecutionProvider()
        val request = dev.fonebrew.contracts.execution.ExecutionRequest(
            id = "req_redaction", targetId = "target-local",
            operation = dev.fonebrew.contracts.execution.TypedOperation(dev.fonebrew.contracts.execution.OperationClass.ONE_SHOT_COMMAND, "echo build-ok"),
            workingRevision = null, environment = dev.fonebrew.contracts.execution.RequestEnvironment(), secretHandles = emptyList(),
            budget = dev.fonebrew.contracts.execution.ExecutionBudget(dev.fonebrew.contracts.execution.SessionReauthorization(60, false)),
            authorityGrant = dev.fonebrew.contracts.execution.AuthorityGrantRef("grant_1", listOf("fb.exec.local_process")),
            expectedOutputs = emptyList(), idempotencyKey = null, sideEffectExternal = false,
            fgsType = dev.fonebrew.contracts.execution.FgsType.NONE, openEnded = false
        )
        ...
        val receipt = kotlinx.coroutines.withTimeout(5_000) {
            provider.observe(handle).let { flow ->
                var found: dev.fonebrew.contracts.execution.ExecutionReceipt? = null
                flow.collect { e -> if (e is dev.fonebrew.contracts.execution.ExecutionEvent.ReceiptReady) found = e.receipt }
                found
            }
        }
