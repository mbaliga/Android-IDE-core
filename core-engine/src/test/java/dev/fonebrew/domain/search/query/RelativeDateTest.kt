...
/** Doc §13 test 10: relative-date arithmetic across a DST boundary and across a timezone
 *  change. Every expected value is built via direct [ZonedDateTime] construction (never by
 *  hand-computed millisecond offsets), so these tests are self-verifying against real DST rules
 *  rather than against a second copy of the same arithmetic bug. */
class RelativeDateTest {
    ...
    @Test fun `naive 24-hour-multiple math would have gotten spring-forward wrong`() {
        // Documents exactly the bug this class exists to avoid: subtracting a flat 86_400_000 ms
    ...
    @Test fun `resolvePoint returns the start of the resolved range`() {
        val zone = ZoneId.of("UTC")
        val now = instant(zone, 2026, 7, 30)
        assertEquals(RelativeDate.resolveRange("-7d", now, zone)!!.startInclusiveMillis, RelativeDate.resolvePoint("-7d", now, zone))
    }
