...
import dev.fonebrew.domain.thread.ObserverScript
import dev.fonebrew.domain.thread.ThreadDeltas
 ...
 * [dev.fonebrew.domain.mirror.AarsoEventLog] (binding constraint 3 / Issue #2) — this is a plain
 ...
 * snapshot" shape [dev.fonebrew.domain.mirror.AarsoEventLog]'s own `settings: () -> AarsoCaptureSettings`
 * constructor param uses) so a live [dev.fonebrew.data.SessionStore.observerEnabled] toggle can be
 ...
 * matching [dev.fonebrew.domain.mirror.Reflection.NONE]'s "materially absent, not faked" rule.
 *
