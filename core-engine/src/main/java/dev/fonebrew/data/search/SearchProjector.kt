 ...
 * convention ([dev.fonebrew.ui.state.ConversationsPresenter], [dev.fonebrew.domain.library.ConversationProjection]):
 * a stateless transform from domain facts to a plain data class, JVM-tested without any of the
 * plumbing that produces those facts at runtime. [dev.fonebrew.data.search.SearchIndexer] (WP6) is
...
object SearchProjector {
    ...
    data class Row(
        ...
        /** [dev.fonebrew.domain.tree.TreeFork.LineageKind] name ("FORK"/"SPAWN"), null exactly when
     ...
     * [dev.fonebrew.data.ConversationsStore] already combines for the (unwired) Chats list —
