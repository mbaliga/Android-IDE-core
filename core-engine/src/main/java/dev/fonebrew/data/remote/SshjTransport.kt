 ...
 * SSH transport handshake completes — but **no secret is sent yet**. Our [dev.fonebrew.domain.remote.RemoteSessionDriver]
...
class SshjTransport(
    ...
    override suspend fun shell(onOutput: (ExecChunk) -> Unit): dev.fonebrew.domain.remote.ShellSession =
        withContext(Dispatchers.IO) {
            val ssh = client ?: error("not connected")
            ...
            object : dev.fonebrew.domain.remote.ShellSession {
