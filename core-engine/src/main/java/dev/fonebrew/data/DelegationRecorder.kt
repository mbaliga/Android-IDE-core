...
import dev.fonebrew.domain.thread.DelegationEvent
import dev.fonebrew.domain.thread.DelegationKind
import dev.fonebrew.domain.thread.DelegationOutcome
import dev.fonebrew.domain.thread.ThreadCodec
import dev.fonebrew.domain.thread.ThreadEvent
import dev.fonebrew.domain.thread.ThreadEventKind
 ...
 * [dev.fonebrew.ui.ChatViewModel.forkFrom]'s KDoc): a queryable [DelegationStore] row, plus an inert,
 * write-only [AarsoEventLog] line (Issue #2 — never read back). Pulled out to its own small class
 * rather than a private `ChatViewModel` method because [dev.fonebrew.ui.loops.LoopRoom]'s
