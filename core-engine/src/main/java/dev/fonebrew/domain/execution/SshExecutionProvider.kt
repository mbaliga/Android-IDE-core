 ...
 * pass) via [dev.fonebrew.data.remote.SshjTransport] in production; JVM tests inject a fake
...
class SshExecutionProvider(
    ...
    override suspend fun start(prepared: PreparedExecution): ExecutionHandle {
        ...
        if (driver.state !is dev.fonebrew.domain.remote.SessionState.Ready) {
    ...
    override suspend fun reconnect(token: ReconnectTokenHandle): ExecutionHandle? {
        ...
        val sessionLost = run.driver.state.let {
            it is dev.fonebrew.domain.remote.SessionState.Failed || it is dev.fonebrew.domain.remote.SessionState.Closed
