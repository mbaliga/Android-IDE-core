 ...
 * `DeviceConnection` lifecycle table, encoded exactly as [dev.fonebrew.domain.workspace.
...
object DeviceConnectionMachine {
    ...
    /** Belt-and-suspenders re-derivation, same discipline [dev.fonebrew.domain.loop.LoopRunDriver]'s `assertLegalEventSequence` test helper established: is `to` reachable from `from` by exactly one of the rows above, independent of any particular event payload? */
