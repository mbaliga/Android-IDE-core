 ...
 * conversation" pair, the other half being [dev.fonebrew.domain.bridge.SpawnBridges] ("Spawn").
 ...
 * ([dev.fonebrew.ui.ChatViewModel.forkFrom]) is responsible for actually inserting [ForkResult.nodes]
 * into the tree store (parent-first order, so the FK-style `parentId` reference is always already
 * present) and for writing the [dev.fonebrew.domain.thread.ThreadEvent]/
 * [dev.fonebrew.domain.thread.ThreadMarker] lineage receipts alongside it.
 */
object TreeFork {

    /** Which "spin off a new conversation" action produced a lineage-tagged root — shared between
     *  [copySubtree] (Fork) and [dev.fonebrew.domain.bridge.SpawnBridges] (Spawn) so both write the
