...
data class LedgerEntry(
     ...
     *  P3) or `"loop"` (a [dev.fonebrew.domain.loop.GraphRunner] step, via
     *  [dev.fonebrew.domain.loop.GraphRunLedger]). Additive: existing chat entries are unaffected. */
    val surface: String = "chat",
    /** The saved [dev.fonebrew.domain.loop.Loop] a loop-surface entry ran from, or `null` for an
