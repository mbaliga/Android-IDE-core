package dev.fonebrew.domain.model

/**
 * Decides which model a session starts with. Pure so the policy is testable and
 * legible in one place.
 *
 * Order: the persisted choice if it still resolves *and* is on-device; else on-device Gemini
 * Nano (AICore) when the onboarding wizard confirmed it's available — no download, so it's the
 * lightest on-device option; else the first downloaded GGUF; else an echo dev stand-in when one
 * is registered (debug builds only); else null — meaning "no model yet", the explicit state the
 * first-run setup card renders.
 *
...
object DefaultModelPolicy {
    ...
    fun resolveActive(specs: List<ModelSpec>, persistedId: String?): ModelSpec? {
        ...
        specs.firstOrNull { it.runtime == Runtime.AICORE_NANO }?.let { return it }
