...
/** Where an [AarsoEvent] line actually gets written. Kept as a pure interface so [AarsoEventLog] stays testable without real file I/O — the on-device implementation ([dev.fonebrew.data.AarsoFileEventSink]) is the only piece that touches a filesystem. */
