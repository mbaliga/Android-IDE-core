 ...
 * [dev.fonebrew.domain.council.Council]'s aggregator-prompt shape (short system framing + a user turn
 ...
 * app's human-as-aggregator thesis stays the default (see [dev.fonebrew.domain.council.Council]'s own
 * KDoc); "choose for me" is the explicit, optional exception at a branch point, same as
 * `autoMergeCouncil` is for the council panel.
 *
 * Pure string-building + parsing, no Android/engine dependency — [dev.fonebrew.ui.ChatViewModel.chooseForMe]
...
object DelegationPrompts {
     ...
     * throughout, e.g. [dev.fonebrew.domain.bridge.SummaryBridges]' "excerpt, not invention").
     */
