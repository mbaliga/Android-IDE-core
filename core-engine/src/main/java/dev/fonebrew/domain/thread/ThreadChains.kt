 ...
 * shape [dev.fonebrew.domain.tree.Conversations] and [dev.fonebrew.domain.tree.SiblingCompares] use.
 * Inputs are exactly what [dev.fonebrew.ui.ChatViewModel] already has in scope: every conversation's
 ...
 * [dev.fonebrew.data.search.SearchProjector] (WP6's other consumer, for the `conv_facets`
 * `lineage_parent`/`lineage_kind`/`chapter_count`/`compaction_count` columns) and
 * [dev.fonebrew.ui.ChatViewModel] both call this the same way.
 *
 * ### Reads Room stores, never the event log
 * Lineage comes from the queryable [ThreadMarkerKind.LINEAGE_SRC] marker
 * [dev.fonebrew.data.ThreadMarkerStore.markLineageSource] writes — never from
 * [dev.fonebrew.domain.mirror.AarsoEventLog], which THREAD_TOPOLOGY_PLAN.md binding constraint 3
 * (Issue #2) keeps write-only. [TreeFork] also stamps the same `lineage.*` keys directly onto a
 * forked root's [dev.fonebrew.domain.MessageNode.metadata], but that copy is for the tree layer's
...
object ThreadChains {
     ...
     *  [dev.fonebrew.data.ThreadMarkerStore.markLineageSource] writes (`srcRootId`/`srcNodeId`/
     *  `lineageKind`). Exposed (not `private`) so [dev.fonebrew.data.search.SearchProjector] can
