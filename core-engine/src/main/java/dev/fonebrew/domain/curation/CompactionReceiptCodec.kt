...
/**
 * `org.json` encode/decode for [Receipt] — the payload [dev.fonebrew.ui.ChatViewModel.runCompaction]
 * hands [dev.fonebrew.data.ReceiptStore.append] wrapped in a
 * `dev.fonebrew.contracts.common.ContractEnvelope<Receipt>` (same shape
 * [dev.fonebrew.domain.authority.AuditedAuthorityEngine] uses for its own receipts), and decodes
 * back via `dev.fonebrew.domain.contracts.EnvelopeCodec.decode` whenever a later turn's
