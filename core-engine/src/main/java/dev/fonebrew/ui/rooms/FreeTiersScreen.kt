...
import dev.fonebrew.domain.cost.FreeTierProvider
...
fun FreeTiersScreen(onClose: () -> Unit) {
    ...
    Column(
        ...
        if (providers.isNotEmpty()) {
            ...
            Spacer(Modifier.height(8.dp))
            for (p in providers) {
                val u = usage[p.id] ?: dev.fonebrew.data.ProviderUsage()
                val tier = dev.fonebrew.domain.cost.FreeTierMatch.match(p, catalog)
                WireBox {
