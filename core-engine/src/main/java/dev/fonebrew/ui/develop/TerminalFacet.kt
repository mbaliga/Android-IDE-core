 ...
 * object** — never paraphrased). Trust stays the user's: [dev.fonebrew.data.DeviceRepo.exec] only
...
fun TerminalFacet() {
    ...
    WireButton(if (running) "Running…" else "Run", enabled = !running && cmd.isNotBlank(), onClick = {
        ...
        scope.launch {
            ...
            runCatching {
                val recipe = dev.fonebrew.domain.device.recipe.DeviceRecipes.shell(
                    dev.fonebrew.domain.device.DeployTarget.Remote(host),
                    command,
                )
