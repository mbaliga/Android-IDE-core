 ...
 * naming rule 9 — "spine" was already taken by [dev.fonebrew.domain.curation.VersionSpines]).
 *
 * One [Dash] per [PathView.Step] on the *active* path — the same path [dev.fonebrew.ui.ChatScreen]
 ...
 * caller already holds as `StateFlow`s on [dev.fonebrew.ui.ChatViewModel] — [ChatThreadPresenter] and
 * [dev.fonebrew.domain.curation.CompactionPreviewPresenter] set the precedent for this shape (data in,
 ...
 * [dev.fonebrew.domain.curation.CompactionContract]'s `isOnVersionSpine` means "on the ancestor path
 * of any [Version]'s tip," computed elsewhere ([dev.fonebrew.domain.curation.VersionSpines]) by
 ...
 * [dev.fonebrew.domain.tree.MessageTree] as an input.
 */
object ThreadRailPresenter {
    ...
    data class LiveGeneration(
        /** Same truth [dev.fonebrew.ui.ChatViewModel.ModelOption.watched] already carries for the active model — true means this turn is reaching a cloud provider (binding rule 2's watched object). */
        val watched: Boolean,
    )

    /**
     * @param steps the active path, root-first — exactly [dev.fonebrew.ui.ChatViewModel.ChatUiState.steps].
     * @param verdicts keyed by message id ([dev.fonebrew.ui.ChatViewModel.verdicts]'s shape).
     * @param bookmarks keyed by message id ([dev.fonebrew.ui.ChatViewModel.messageBookmarks]'s shape).
     * @param versionsByTip keyed by [Version.branchTipMsgId] ([dev.fonebrew.ui.ChatViewModel.versionsByTip]'s shape).
     * @param directives keyed by message id ([dev.fonebrew.ui.ChatViewModel.compactionDirectives]'s shape).
     * @param markersByAnchor keyed by [ThreadMarker.anchorMsgId] ([dev.fonebrew.ui.ChatViewModel.threadMarkers]'s shape — already pre-filtered to non-null anchors there).
