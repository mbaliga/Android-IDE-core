...
import dev.aarso.domain.curation.CompactionAgent
import dev.aarso.domain.curation.MessageFate
import dev.aarso.domain.curation.ResolvedFidelity
import kotlinx.coroutines.flow.collect

/**
 * The model-call half of the Compaction Contract — [dev.aarso.domain.curation.CompactionEngine]'s
 ...
 * [dev.aarso.domain.curation.CompactionEngine.run] actually calls an agent for.
 *
 ...
 * responsibility [dev.aarso.ui.ChatViewModel.runTurn] already uses for the main chat loop).
 */
class EngineCompactionAgent(private val engine: InferenceEngine) : CompactionAgent {
     ...
     *   [dev.aarso.domain.curation.CompactionEngine.run] never calls [compact] for either (it
