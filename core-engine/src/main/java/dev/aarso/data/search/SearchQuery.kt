    ...
    fun search(
        ...
        if (lexicalText.isBlank()) {
            ...
            return surviving.take(resultLimit).map { c ->
                SearchHit(doc = c.doc, score = 0.0, matchedIn = dev.aarso.domain.search.MatchedIn.CONTENT, highlights = emptyList())
            }
        }
