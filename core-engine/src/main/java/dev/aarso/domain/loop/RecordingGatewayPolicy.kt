 ...
 * [dev.aarso.domain.thread.DelegationPrompts.parseChoice] applies on the model-pick surface.
 *
 ...
 * read once a run finishes; [dev.aarso.ui.loops.LoopRoom] persists each entry via
 * [dev.aarso.data.DelegationRecorder] only after the run completes — the same "log to the tree
 * only when the run is done" discipline it already applies to [GraphRunResult] via `GraphRunLog`.
 *
 * A `DELEGATION` recorded this way has no [dev.aarso.domain.thread.DelegationEvent.rootId] /
 ...
 * so [dev.aarso.ui.ChatViewModel]'s per-conversation outcome correlation never resolves these;
