 ...
 * time, not a duplicate entry. In-memory here, not Room-backed -- [dev.aarso.data.RoomWorkspaceJournal]
...
class DraftEditJournal {
    ...
    /** Deterministic replay: last write wins per field path, in append (sequence) order -- same "materialize" posture as [dev.aarso.domain.workspace.BufferReplay] (WP-3). */
