...
/**
 * Wire shape for the five new [dev.aarso.domain.mirror.AarsoEventKind] values
 ...
 * `dev.aarso.contracts.common.ContractEnvelope<T>` the same way `ErrorEnvelope` is — mirroring
 * `dev.aarso.domain.contracts.EnvelopeCodec`'s own convention, not `CurationStore`'s Room-facing
