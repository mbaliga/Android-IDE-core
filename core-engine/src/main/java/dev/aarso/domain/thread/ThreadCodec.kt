...
import dev.aarso.domain.contracts.extractUnknownFields
import dev.aarso.domain.contracts.mergeUnknownFields
import dev.aarso.domain.contracts.optStringOrNull
import org.json.JSONArray
import org.json.JSONObject
import java.time.Instant

/**
 * `org.json` encode/decode for every `dev.aarso.domain.thread` shape, mirroring the three
 * `schemas/thread/` documents (thread-event, thread-marker, thread-graph) field-for-field — same
 * discipline `dev.aarso.domain.contracts.EnvelopeCodec` uses for the `schemas/common/` ones, and reusing
 * that file's [dev.aarso.domain.contracts.extractUnknownFields]/[dev.aarso.domain.contracts.mergeUnknownFields]
 ...
 * additionally get mapped to/from their Room entities by [dev.aarso.data.ThreadMarkerStore]/
 * [dev.aarso.data.DelegationStore] (WP1), a separate, simpler `Long`-epoch-millis mapping that
...
/** Null-safe string-list reader for a JSON array of strings, defaulting to empty (mirrors [dev.aarso.domain.contracts.optStringOrNull]'s "absent -> default" shape for a list-typed field). */
