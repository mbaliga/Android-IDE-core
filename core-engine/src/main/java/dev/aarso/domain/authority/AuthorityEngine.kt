...
class AuthorityEngine(
    ...
    fun evaluate(
        ...
        purpose: String? = null,
        escalateToRung: dev.aarso.contracts.authority.AuthorityRung? = null,
    ): AuthorityDecision {
        val decidedAt = now()
        ...
        if (mitigationMode == "REDACTION" || mitigationMode == "SANDBOX") {
            ...
            return AuthorityDecision.AllowWithRedactionOrSandbox(
                ...
                redactionOrSandbox = dev.aarso.contracts.authority.RedactionOrSandboxDetail(
                    mode = dev.aarso.contracts.authority.RedactionOrSandboxMode.valueOf(mitigationMode), detail = detail
                )
            )
        }
        ...
        val needsConfirmation = when (grant.confirmationPolicy.mode) {
            dev.aarso.contracts.authority.ConfirmationMode.NEVER_REQUIRED -> false
            dev.aarso.contracts.authority.ConfirmationMode.ALWAYS_REQUIRED -> true
            dev.aarso.contracts.authority.ConfirmationMode.REQUIRED_ABOVE_RUNG ->
    ...
    private fun ancestorHoldsCoveringGrant(
        principal: Principal, capabilityId: String, resourceScope: ResourceScope, requiredRung: dev.aarso.contracts.authority.AuthorityRung,
    ): Boolean {
