...
class LocalProcessExecutionProvider(
     ...
     * [dev.aarso.domain.workspace.LocalWorkspaceProvider.watch]'s pattern) and emits a
    ...
    override suspend fun reconnect(token: ReconnectTokenHandle): ExecutionHandle? {
        ...
        return ExecutionHandle(
            ...
            reconnectAttempt = dev.aarso.contracts.execution.ReconnectAttempt(attemptedAtUtc = now(), outcome = ReconnectOutcome.ESTABLISHED),
