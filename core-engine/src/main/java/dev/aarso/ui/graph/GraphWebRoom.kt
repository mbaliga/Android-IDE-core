...
import dev.aarso.domain.thread.ThreadGraphJson
 ...
 * one step). [dev.aarso.domain.disclosure.Surface.GRAPH_DEEP] gates the entry point itself; this
 ...
 * (`scripts/build-graph-room.js`'s output — see [dev.aarso.ui.graph.GraphRoomAssetTest] for what
 * that file is asserted to hold) and pushes the current [graph] into it once via
 * `window.__graphRoom.load(json)`, using
 * [dev.aarso.domain.thread.ThreadGraphJson.toJsonString] — the exact forward pointer that
 ...
 *   [dev.aarso.ui.graph.GraphRoomAssetTest]'s KDoc) can reach the network, full stop.
