 ...
 * **active thread's rendered text** (each [dev.aarso.ui.state.ChatThreadPresenter.ThreadRow]'s
 * `renderText`) — no Android, no clock.
 *
 * One deliberate scope cut from a literal reading of "reuse the [dev.aarso.domain.search
 * .Segmenter]/[dev.aarso.domain.search.LexicalSearch] pipeline": that pipeline is a *document*
 ...
 * [dev.aarso.domain.search.query.QueryParser] — an invalid `/regex/` degrades to
