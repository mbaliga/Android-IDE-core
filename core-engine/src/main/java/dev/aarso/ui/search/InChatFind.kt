 ...
 * [dev.aarso.ui.ChatScreen]'s wiring). Row-level highlighting only, not sub-string spans inside
 * the rendered turn — the message body renders through the mikepenz markdown composable
 * ([dev.aarso.ui.ChatScreen]'s `Markdown(...)` call, per this repo's Compose↔markdown pin), which
